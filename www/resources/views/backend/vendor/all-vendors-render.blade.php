<div class="table-responsive">
    <table class="table table-sm table-bordered table-striped table-hover onex-datatable nowrap" id="dataTable" style="width: 100%;">
        ...
        @if(!empty($data) && count($data))
            ...
            @foreach($data as $key => $value)
                <tr>
                    ...
                    <td>
                        @if(!empty($value->userProfile->gst_no))
                            {{ $value->userProfile->gst_no }}
                        @endif
                    </td>
                    ...
                    <td class="action-col">
                        <div class="btn-group">
                            <a href="{{ route('vendor.edit', array('id' => $value->hash_id)) }}" class="btn edit-user-btn"><i class="far fa-edit text-success"></i></a>
                            <a href="{{ route('vendor.delete', array('id' => $value->hash_id)) }}" class="btn remove-user-btn"><i class="far fa-trash-alt text-danger"></i></a>
                        </div>
                    </td>
                </tr>
                @php $sl++; @endphp
        ...
        @else
            <tr>
                <td colspan="8">No vendors found! Please create vendor or supplier</td>
            </tr>
        @endif
        </tbody>
    </table>
