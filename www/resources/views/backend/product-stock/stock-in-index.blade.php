...
@section('page_breadcrumb')
    ...
    <li class="breadcrumb-item active">Today Stock In</li>
@endsection

@section('content_title', 'Today Stock In - ' . date('d F, Y'))
@section('content_buttons')
    <a href="{{ route('stock.add') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Add Stock</a>
@endsection
            ...
            <table class="table table-sm table-bordered table-striped table-hover nowrap" id="todayStockInTable" style="width: 100%;">
                ...
                @else
                    <tr>
                        ...
                        <td style="display: none;"></td>
                        <td style="display: none;"></td>
                        <td colspan="10">No records found for today. Please add stock for the day</td>
                    </tr>
                @endif
                </tbody>
            </table> 
...
@section('content_footer')
    @if(!empty($data) && count($data))
    <div class="row">
        <div class="col-md-12 text-right">Today Total Stock In Amount: <strong>{{ $todayTotal }}</strong></div>
    </div>
    @endif
@endsection
